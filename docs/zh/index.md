---
layout: home

hero:
  name: "流行演唱完全指南"
  text: "为认真学习的歌者而写的参考手册"
  tagline: 认真训练，然后让训练消融于歌声之中。
  actions:
    - theme: brand
      text: 开始阅读
      link: /intro
    - theme: alt
      text: 术语表
      link: /appendix

features:
  - title: 第一部分 — 基础
    details: 嗓音的生理结构、体态、呼吸基础、音域、换声区（passaggio）与嗓音热身。
    link: /part1
  - title: 第二部分 — 流行音色
    details: 流行演唱的核心悖论、扩音筒塑形、元音调整、起音与鸣音（twang）。
    link: /part2
  - title: 第三部分 — 声区与混声
    details: 胸声、头声、假声、换声点，以及将混声发展为流行演唱核心引擎的方法。
    link: /part3
  - title: 第四部分 — 气息与力量
    details: 举重运动员式支撑、身体锚定（anchoring），以及如何避免过度换气的陷阱。
    link: /part4
  - title: 第五部分 — 风格
    details: 颤音、气泡音、即兴花腔（run/riff）、辅音节省、风格性气息感与装饰音。
    link: /part5
  - title: 第六部分 — 麦克风
    details: 近讲效应、距离调控、麦克风角度，以及现场与录音棚技术的差异。
    link: /part6
  - title: 第七部分 — 练习与进步
    details: 练习课结构、日常节奏、录音自检、进步的信号与问题的信号。
    link: /part7
  - title: 第八部分 — 嗓音健康
    details: 补水、睡眠、胃酸反流、过敏、疲劳嗓音与受损嗓音的区别，以及何时就医。
    link: /part8
  - title: 第九部分 — 几句实在话
    details: 关于老师、进步的节奏、找到自己的声音，以及这门技艺的本质。
    link: /part9
---
